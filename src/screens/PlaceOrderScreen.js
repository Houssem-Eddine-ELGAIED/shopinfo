...
export default function PlaceOrderScreen() {
  ...
  const placeOrderHandler = async () => {
    try {
      ...
      // Envoi de la commande avec les informations nécessaires (dont `isPaid` et `isDelivered`)
      const { data } = await Axios.post(
        '/api/orders',
        {
          orderItems: cart.cartItems,
          shippingAddress: cart.shippingAddress,
          paymentMethod: cart.paymentMethod || 'Cash on Delivery', // Utilisation d'une valeur par défaut
          itemsPrice: cart.itemsPrice,
          shippingPrice: cart.shippingPrice,
          taxPrice: cart.taxPrice,
          totalPrice: cart.totalPrice,
          isPaid: true, // Commande payée
          paidAt: new Date().toISOString(), // Date de paiement
          isDelivered: true, // Commande livrée
          deliveredAt: new Date().toISOString(), // Date de livraison
        },
        {
          headers: {
